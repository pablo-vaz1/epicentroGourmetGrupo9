...
public class PedidoDao {
	...
	public double calcularGananciasPorUnidadYFechas(UnidadVenta unidadVenta, LocalDate fechaDesde,
		...
		try {
			...
			String hql = "SELECT SUM(i.cantidad * (pl.precio - pl.costoProduccion)) " + "FROM Pedido p "
					+ "JOIN p.items i " + "JOIN i.plato pl " + "WHERE p.unidadVenta = :unidadVenta "
					+ "AND p.fecha BETWEEN :fechaDesde AND :fechaHasta";

			var query = session.createQuery(hql, Double.class).setParameter("unidadVenta", unidadVenta)
